...
BEGIN
    ...
    SELECT
        ...
        (
            SELECT
                r.TaskName, r.ObjectName, r.WorkspaceName, r.TaskType, r.ParametersJson,
            ...
            -- INCLUDE_NULL_VALUES: without it, FOR JSON PATH omits the ParametersJson key
            -- entirely for any Task where it's NULL, instead of emitting it as JSON null.
            -- The wave-runner pipelines' `@item().ParametersJson` expression then fails with
            -- "property 'ParametersJson' doesn't exist" for that Task, since ADF/Fabric's
            -- expression language throws on a genuinely missing property rather than treating
            -- it as undefined -- this guarantees the key is always there to reference.
